...
public class ExportUserLogsTest extends SetUp {

	final static Logger logger = Logger.getLogger(ExportUserLogsTest.class);
	...
	public void exportUserLogsTest() throws Exception {
		...
		test = extent.createTest("exportUserLogsTest");
		setExtentTest(test);
		...
		getExtTest().log(Status.INFO, "Clicking on Export User Logs tab.......");
		exportPage.clickOnExportUserLogsTab();

		getExtTest().log(Status.INFO, "Selecting duration.......");
		exportPage.selectDuration();
		
		getExtTest().log(Status.INFO, "Selecting Role as Sales.......");
		...
		getExtTest().log(Status.INFO, "Selecting Export type as User Logs.......");
		...
		Assert.assertEquals(users, "User Logs - Export 1", "Not matched");
		
		wait.until(ExpectedConditions.visibilityOfElementLocated(exportPage.exportStatus));
		String exportStatus = driver.findElements(exportPage.userLogExportStatus).get(0).getText();
