...
public class ExportInvoices_ItemizedBill_EmailTest extends SetUp {

	final static Logger logger = Logger.getLogger(ExportInvoices_ItemizedBill_EmailTest.class);
	...
	public void exportInvoices_ItemizedBill_EmailTest() throws Exception {
		...
		test = extent.createTest("exportInvoices_ItemizedBill_EmailTest");
		setExtentTest(test);
		...
		getExtTest().log(Status.INFO, "Selecting Email from Type dropdown.......");
		...
		getExtTest().log(Status.INFO, "Selecting Export type as Itemised Bill(Email).......");
		...
		String itemisedBill_Email = driver.findElements(exportPage.exportLabel).get(0).getAttribute("innerHTML");
		System.out.println(itemisedBill_Email);

		getExtTest().log(Status.INFO, "Validating that the latest export is made for Itemized Bill Emails.......");
		Assert.assertEquals(itemisedBill_Email, "Itemized Bill Emails - Export 1", "Not matched");
