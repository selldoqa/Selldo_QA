...
public class ExportCallsTest extends SetUp {

	final static Logger logger = Logger.getLogger(ExportCallsTest.class);
	...
	public void exportCallsTest() throws Exception {
		...
		test = extent.createTest("exportCallsTest");
		setExtentTest(test);
		...
		getExtTest().log(Status.INFO, "Clicking on Export Calls tab.......");
		...
		getExtTest().log(Status.INFO, "Selecting Export type as Calls.......");
		...
		String call = driver.findElements(exportPage.exportLabel).get(0).getAttribute("innerHTML");
		System.out.println(call);

		getExtTest().log(Status.INFO, "Validating that the latest export is made for Calls.......");
		Assert.assertEquals(call, "Calls - Export 1", "Not matched");
