...
public class ExportInvoices_ItemizedBill_CalllTest extends SetUp {

	final static Logger logger = Logger.getLogger(ExportInvoices_ItemizedBill_CalllTest.class);
	...
	public void exportInvoices_ItemizedBill_CalllTest() throws Exception {
		...
		test = extent.createTest("exportInvoices_ItemizedBill_CalllTest");
		setExtentTest(test);
		...
		getExtTest().log(Status.INFO, "Selecting Call from Type dropdown.......");
		...
		getExtTest().log(Status.INFO, "Selecting Export type as Itemised Bill(Call).......");
		...
		String itemisedBill_Call = driver.findElements(exportPage.exportLabel).get(0).getAttribute("innerHTML");
		System.out.println(itemisedBill_Call);

		getExtTest().log(Status.INFO, "Validating that the latest export is made for Itemized Bill Calls.......");
		Assert.assertEquals(itemisedBill_Call, "Itemized Bill Calls - Export 1", "Not matched");
