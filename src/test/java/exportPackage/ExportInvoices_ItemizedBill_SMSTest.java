...
public class ExportInvoices_ItemizedBill_SMSTest extends SetUp {

	final static Logger logger = Logger.getLogger(ExportInvoices_ItemizedBill_SMSTest.class);
	...
	public void exportInvoices_ItemizedBill_SMSTest() throws Exception {
		...
		test = extent.createTest("exportInvoices_ItemizedBill_SMSTest");
		setExtentTest(test);
		...
		getExtTest().log(Status.INFO, "Selecting Sms from Type dropdown.......");
		...
		getExtTest().log(Status.INFO, "Selecting Export type as Itemised Bill(Sms).......");
		...
		String itemisedBill_Sms = driver.findElements(exportPage.exportLabel).get(0).getAttribute("innerHTML");
		System.out.println(itemisedBill_Sms);

		getExtTest().log(Status.INFO, "Validating that the latest export is made for Itemized Bill Smses.......");
