...
public class ExportVirtualNumbersTest extends SetUp {

	final static Logger logger = Logger.getLogger(ExportVirtualNumbersTest.class);
	...
	public void exportVirtualNumbersTest() throws Exception {
		...
		test = extent.createTest("exportVirtualNumbersTest");
		setExtentTest(test);
		...
		getExtTest().log(Status.INFO, "Clicking on Export Virtual Number tab.......");
		...
		getExtTest().log(Status.INFO, "Selecting Export type as Virtual number.......");
		...
		String virtualNumber = driver.findElements(exportPage.exportLabel).get(0).getAttribute("innerHTML");
		System.out.println(virtualNumber);

		getExtTest().log(Status.INFO, "Validating that the latest export is made for Virtual Numbers.......");
		Assert.assertEquals(virtualNumber, "Virtual Numbers - Export 1", "Not matched");
