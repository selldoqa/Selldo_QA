...
public class ExportSitevisitsTest extends SetUp {

	final static Logger logger = Logger.getLogger(ExportSitevisitsTest.class);
	...
	public void exportSitevisitsTest() throws Exception {
		...
		test = extent.createTest("exportSitevisitsTest");
		setExtentTest(test);
		...
		getExtTest().log(Status.INFO, "Clicking on Export Site Visits tab.......");
		...
		getExtTest().log(Status.INFO, "Selecting Export type as Site Visits.......");
		...
		String sitevisits = driver.findElements(exportPage.exportLabel).get(0).getAttribute("innerHTML");
		System.out.println(sitevisits);

		getExtTest().log(Status.INFO, "Validating that the latest export is made for Site Visits.......");
