...
public class LogOfflineCall_InboundAnsweredTest extends SetUp {

	final static Logger logger = Logger.getLogger(LogOfflineCall_InboundAnsweredTest.class);
	...
	public void appLogin() throws Exception {
		...
		login.login(
				property.getProperty("name") + "+"
						+ property.getProperty("user_email_logOfflineCall_InboundAnsweredTest"),
	...
	public void logOfflineCall_InboundAnsweredTest() throws Exception {
		...
		test = extent.createTest("logOfflineCall_Inbound_Answered_Test");
		setExtentTest(test);
		...
		getExtTest().log(Status.INFO, "Selecting Duration unit as Minutes......");
		...
		Assert.assertEquals(croppedDirectionText, "Incoming   |   Offline   |   Ans", "text not matching");
		...
		getExtTest().log(Status.INFO, "Verifying text under View notes....");
